...
export default function Header() {
  ...
  return (
      <header className="bg-gray-5 h-20 font-inter relative z-40">
          ...
          <div className="flex justify-between w-full">
            ...
            <div>
              ...
              >
                ...
                />
                <div
                    className="text-xl md:text-xl lg:text-[1.5rem] xl:text-3xl pl-5 font-semibold text-mallorca-purple">
                  {t("app.title")}
                </div>
              </Link>
            </div>
              ...
              <div className="h-full" id="navbarToggle">
                <ul className="rounded min-w-36 w-full h-full">
                  <li className="relative w-full h-full">
                    ...
                    >
                      <div
                          className="flex justify-between items-center gap-2 h-full text-mallorca-purple/75 hover:bg-mallorca-purple/10 rounded-lg px-3">
                        <img
            ...
            <div className="flex lg:hidden items-center">
              <button
                  ...
                  className="rounded cursor-pointer p-2 text-center hover:bg-mallorca-purple/50"
        ...
        {mobileOpen && (
            <div
                ref={mobileRef}
                id="mobile-nav"
                className="lg:hidden px-5 pb-4 absolute left-0 right-0 top-20 z-50 bg-white shadow-md"
            >
              <div className="pt-2 pb-3 space-y-1 border-t border-gray-200">
                {/* Language selector */}
                <div className="py-2">
                  {Object.keys(language_title_img_map).map((key) => {
                    const lang = language_title_img_map[key];
                    const isSelected = i18n.language === key;
                    return (
                        <button
                        ...
                        >
                          <img src={lang?.img} alt={lang?.imgAlt} className="h-5"/>
                          <span className="text-mallorca-purple/75">
                      {lang?.language}
                    </span>
                        </button>
                    );
                  ...
                  {auth?.accessToken ? (
                      <>
                        ...
                        <button
                            ...
                            className="w-full text-left p-2 rounded bg-mallorca-purple text-white mt-2"
                        >
                          {t("nav.signOutBtn")}
                        </button>
                      </>
                      ...
                      <>
                        ...
                        <Link
                            to="/register"
                            onClick={() => setMobileOpen(false)}
                            className="block p-2 rounded bg-mallorca-purple text-white mt-2"
                        >
                          {t("nav.registerBtn")}
                        </Link>
                      </>
