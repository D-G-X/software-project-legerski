...
function useGetLicenseData(
  ...
  const { data: licenseResponse } = useGetLicense(matching?.id as number, {
    axios: {
      headers: {
        Authorization: `Bearer ${auth?.accessToken}`,
      },
...
export default function ApplicationDetails({
  ...
  function handleFinishApplication(status: string, applicationId: number) {
    if (status === "DRAFT") {
  ...
  const onDownload = async () => {
    await downloadPdfOfficialDocument({
      downloadFileName: t("applicationDetails.licenseCertificate.fileName", {
      ...
      text:
        t("applicationDetails.licenseCertificate.text.line1", {
          firstName: userData?.firstName ?? "",
          lastName: userData?.lastName ?? "",
        }) +
        "\n" +
        ...
        t("applicationDetails.licenseCertificate.text.line5", {
          cadastralReference:
            applicationData?.cadastral_reference ?? "Unknown Reference",
        }) +
        "\n" +
        ...
        t("applicationDetails.licenseCertificate.text.line7", {
          expiresAt:
            formatDateLong(licenseData?.expires_at, t) ?? "Unknown Date",
        }) +
        "\n\n" +
            ...
            <div className="font-inter min-w-80vb max-w-200vb">
                    ...
                    <div className="bg-gray-50 rounded-lg p-6 mt-2 shadow space-y-2.5 text-gray-700">
                      ...
                      <div className="flex justify-between min-w-lg">
                        ...
                        >
                          {t(
                            formatStatusLabel(
                              "applicationDetails.licenseStatus.",
                      ...
                      <div className="flex justify-between min-w-lg">
                        <span>
                          {t(
                            "applicationDetails.index.license.expiresOnLabel",
                          ) + ": "}
                        </span>
                        <span>
                          {formatDateLong(licenseData?.expires_at, t)}
                        </span>
                      </div>
                    </div>
                  </>
                  ...
                  <div className="bg-gray-50 rounded-lg p-6 mt-2 shadow space-y-2.5 text-gray-700">
                    <div className="flex justify-between min-w-lg">
                      ...
                      >
                        {t(
                          formatStatusLabel(
                            "applicationDetails.documentStatus.",
                            documentData?.status,
                          ),
                        )}
                      </span>
              ...
              <div className="my-8 flex flex-line items-center justify-center gap-4">
                <>
                  ...
                  {["ACTIVE", "EXPIRED"].includes(
                    ...
                    !licenseReleased && (
                      <button
                        ...
                        onClick={() => {
                          navigate(
                            `/license-application-request/renew/${applicationData.id}`,
                          );
                        }}
                        className={
                          "bg-mallorca-purple text-white px-10 py-2 rounded-md min-w-48 max-w-96 font-medium text-lg hover:bg-mallorca-purple/90"
                        }
                      >
                  ...
                  {["ACTIVE"].includes(licenseData?.license_status ?? "") &&
                    !licenseReleased && (
                      <button
                        type="submit"
                        onClick={() => setIsPopupOpen(true)}
                        className={
                          "bg-red-500 text-white  px-10 py-2 rounded-md min-w-48 max-w-96 font-medium text-lg hover:bg-red-700"
                        }
                      >
