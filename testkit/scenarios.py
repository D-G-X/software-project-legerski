...
from tests.e2e_tests.ui_runner import ui_create_ballot_period, ui_login, ui_logout, ui_register, \
    ...
    ui_view_application_details_edit, ui_view_user_details, ui_edit_user_details
...
def DASHBOARD_VIEW_EDIT_DETAILS() -> Scenario:
    return Scenario(
        name="dashboard_edit_user_details",
        steps=[
            lambda p, ctx: ui_register(p, ctx.user),
            lambda p, ctx: ui_login(p, ctx.user),
            lambda p, ctx: ui_edit_user_details(p),
            lambda p, ctx: ui_logout(p, ctx.user),
        ],
    )